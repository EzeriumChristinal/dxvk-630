...
namespace dxvk {
  ...
  void DxvkBarrierBatch::mergeRanges() {
    if (m_imageBarriers.size() < 4)
      return;
    ...
    for (uint32_t i = 0; i < v.size(); ) {
      ...
      while (j < v.size()
        ...
        && v[j].dstStageMask == v[i].dstStageMask
        && v[j].srcAccessMask == v[i].srcAccessMask
        && v[j].dstAccessMask == v[i].dstAccessMask
