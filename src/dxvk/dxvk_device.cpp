...
namespace dxvk {
  ...
  void DxvkDevice::lockPipelineCache() {
    m_pipelineCacheLock.lock();
  }


  void DxvkDevice::unlockPipelineCache() {
    m_pipelineCacheLock.unlock();
  }


  VkPipelineCache DxvkDevice::getPipelineCache() const {
  ...
  void DxvkDevice::initPipelineCache() {
    ...
    if (!paths.directory.empty()) {
      std::string cacheFile = paths.directory + env::PlatformDirSlash + ".dxvk.pipeline";
      util::File file(cacheFile, util::FileFlag::AllowRead);
    ...
    if (m_vkd->vkCreatePipelineCache(m_vkd->device(), &info, nullptr, &m_pipelineCache) != VK_SUCCESS)
      m_pipelineCache = VK_NULL_HANDLE;
  }


  void DxvkDevice::destroyPipelineCache() {
    ...
    if (!paths.directory.empty()) {
      size_t dataSize = 0;
      VkResult vr = m_vkd->vkGetPipelineCacheData(m_vkd->device(), m_pipelineCache, &dataSize, nullptr);

      if (vr == VK_SUCCESS && dataSize > 0) {
        std::vector<char> data(dataSize);
        vr = m_vkd->vkGetPipelineCacheData(m_vkd->device(), m_pipelineCache, &dataSize, data.data());

        if (vr == VK_SUCCESS) {
          std::string cacheFile = paths.directory + env::PlatformDirSlash + ".dxvk.pipeline";
          util::File file(cacheFile, util::FileFlags(util::FileFlag::AllowWrite, util::FileFlag::Truncate));

          if (file)
            file.write(0, dataSize, data.data());
        }
      }
    }
