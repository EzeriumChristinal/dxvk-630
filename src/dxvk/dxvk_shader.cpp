...
namespace dxvk {
  ...
  DxvkShaderPipelineLibraryHandle DxvkShaderPipelineLibrary::compileShaderPipelineLocked() {
    ...
    bool compiledBefore = m_compiledOnce;
    ...
    if (!std::exchange(m_compiledOnce, true)) {
  ...
  VkPipeline DxvkShaderPipelineLibrary::compileVertexShaderPipeline(
    ...
    VkPipeline pipeline = VK_NULL_HANDLE;
    m_device->lockPipelineCache();
    VkResult vr = vk->vkCreateGraphicsPipelines(vk->device(), m_device->getPipelineCache(), 1, &info, nullptr, &pipeline);
    m_device->unlockPipelineCache();
  ...
  VkPipeline DxvkShaderPipelineLibrary::compileComputeShaderPipeline(
    ...
    VkPipeline pipeline = VK_NULL_HANDLE;
    m_device->lockPipelineCache();
    VkResult vr = vk->vkCreateComputePipelines(vk->device(), m_device->getPipelineCache(), 1, &info, nullptr, &pipeline);
    m_device->unlockPipelineCache();
