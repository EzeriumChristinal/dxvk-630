...
namespace dxvk {
  ...
  bool DxvkMemoryAllocator::enableDefrag() const {
    ...
    if (option == Tristate::Auto) {
      // Disable defrag on Gen9 iGPUs: UMA makes defrag pointless
      // (no separate VRAM to compact into) and overhead wastes
      // limited memory bandwidth on shared system RAM.
      if (m_device->adapter()->isGen9LowPower())
        return false;
